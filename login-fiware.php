...
$_SESSION['WPOA']['PROVIDER'] = 'Fiware';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_fiware_api_enabled'));
define('CLIENT_ID', get_option('wpoa_fiware_api_id'));
define('CLIENT_SECRET', get_option('wpoa_fiware_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', 'profile'); // PROVIDER SPECIFIC: 'profile' is the minimum scope required to get the user's id from fiware
define('URL_AUTH', "https://accounts.fiware.com/o/oauth2/auth?");
define('URL_TOKEN', "https://accounts.fiware.com/o/oauth2/token?");
define('URL_USER', "https://www.fiwareapis.com/plus/v1/people/me?");
...
function get_oauth_token($wpoa) {
	...
	// parse the result:
	$result_obj = json_decode($result, true); // PROVIDER SPECIFIC: fiware encodes the access token result as json by default
	$access_token = $result_obj['access_token']; // PROVIDER SPECIFIC: this is how fiware returns the access token KEEP THIS PROTECTED!
	$expires_in = $result_obj['expires_in']; // PROVIDER SPECIFIC: this is how fiware returns the access token's expiration
...
function get_oauth_identity($wpoa) {
	...
	$params = array(
		'access_token' => $_SESSION['WPOA']['ACCESS_TOKEN'], // PROVIDER SPECIFIC: the access_token is passed to fiware via POST param
	);
	$url_params = http_build_query($params);
	...
	$oauth_identity['id'] = $result_obj['id']; // PROVIDER SPECIFIC: fiware returns the user's OAuth identity as id
	//$oauth_identity['email'] = $result_obj['emails'][0]['value']; // PROVIDER SPECIFIC: fiware returns an array of email addresses. To respect privacy we currently don't collect the user's email address.
