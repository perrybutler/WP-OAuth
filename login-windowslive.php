...
function get_oauth_token() {
	...
	switch (strtolower(HTTP_UTIL)) {
		...
		case 'stream-context':
			...
			if ($result === false) {
				endLogin("Sorry, we couldn't log you in. Could not retrieve access token via stream context. Please notify the admin or try again later.");
			}
			break;
	}
	// parse the result:
...
function get_oauth_identity() {
	...
	switch (strtolower(HTTP_UTIL)) {
		...
		case 'stream-context':
			...
			$result = @file_get_contents($url, false, $context);
			if ($result === false) {
				endLogin("Sorry, we couldn't log you in. Could not retrieve user identity via stream context. Please notify the admin or try again later.");
			}
			$result_obj = json_decode($result, true);
			break;
	}
